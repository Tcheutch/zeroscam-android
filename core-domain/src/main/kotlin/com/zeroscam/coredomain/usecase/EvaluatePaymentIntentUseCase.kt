...
class EvaluatePaymentIntentUseCase(
    ...
    fun evaluate(
        ...
        val afterTechnicalSignalsEscalation =
            escalateForTechnicalSignals(
                afterKnownScamEscalation,
            )

        // 4) Ajustement par la politique Afrique / canal / plan
        val policyAdjusted =
            adjustForPlanRegionAndChannel(
     ...
     */
    private fun escalateForKnownScamDestination(currentResult: DetectionResult): DetectionResult {
        val hasKnownScamReason =
            currentResult.reasons.any { reason ->
        ...
        val targetConfidence =
            if (currentResult.riskLevel >= RiskLevel.HIGH) {
     ...
     */
    private fun escalateForTechnicalSignals(currentResult: DetectionResult): DetectionResult {
    ...
    private fun classifyChannelRisk(paymentIntent: PaymentIntent): ChannelRiskBucket {
     ...
     *   - sealed class
     */
    private fun isHighTouchPlan(plan: SubscriptionPlan?): Boolean {
        val planString =
            plan
                ?.toString()
                ?.lowercase()
